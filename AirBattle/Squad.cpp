...
namespace AirBattle {
	...
	Squad::~Squad() {
		while (!plane.empty()) {
			other_vector<Plane*>::iterator i = plane.end() - 1;
			delete (*i);
			plane.erase(i);
		}
	}
	...
	double Squad::get_disguise()const {
		double r = 0.0;
		for (other_vector<Plane*>::iterator i = plane.begin(); i != plane.end(); i++) {
	...
	Plane*& Squad::get_plane(const int& index) {
		...
		other_vector<Plane*>::iterator i = plane.begin();
		i += index;
		return *i;
	}
	...
	double Squad::go(const double& xx, const double& yy,float& time,const double& multiplier) {
		...
		x += speed*time*(xx - x) / distance * multiplier;
		double new_y = speed*time*(yy - y) / distance * multiplier;
		y += new_y;
		for (other_vector<Plane*>::iterator i = plane.begin(); i != plane.end(); i++) {
			(*i)->set_x(x);
	...
	void Squad::attack_squad(Squad& s, Table& my, Table& enemy, const int& t) {
		srand(time(0));
		other_vector<Plane*>::iterator j = s.plane.begin();
		for (auto i : plane) {

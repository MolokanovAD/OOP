...
int main() {
	...
	std::vector<Button*> buttons, actions, numbers;
	...
	for (int i = 0; i < 4; i++) {//создание кнопок
		int y = w - 185 + i * 45;
	...
	std::vector<SquadInfo*> squads, en_squads;//векторы со звеьями пользователя и врага соответственно
	for (int i = 0; i < M.get_table(true).get_size(); i++) {
		Squad* S = &M.find_squad(squad_name[i], true);
		squads.push_back(new SquadInfo(*S, squad_name[i], T, font, true));
	}
	for (int i = 0; i < M.get_table(false).get_size(); i++) {
	...
	while (window.isOpen()) {
				...
				if (i->is_attacking_squad()) {//атака звена
					...
					if (i->monster_kill()) {
						for (auto k : squads) {//если еще кто-то стрелял по этому звену прекратить огонь
							if (k->is_attacking_squad() && i != k && k->get_s_goal() == i->get_s_goal()) {
								k->set_squad_attack(false);
								k->set_s_goal(nullptr);
							}
						}
						int j = 0;
						for (std::vector<SquadInfo*>::iterator ittt = en_squads.begin(); ittt != en_squads.end(); ittt++) {//удаление уничтоженного звена
							if (i->get_s_goal() == (*ittt)) {
								en_squads.erase(ittt + j);
								break;
							}
						}
					...
					if (i->frag()) {
						for (auto k : squads) {//если еще кто-то стрелял по этому самолету прекратить огонь
							if (k->is_attacking_plane() && i != k && k->get_p_goal() == i->get_p_goal()) {
								...
								k->set_command(0);
								k->set_plane_attack(false);
								k->set_p_goal(nullptr);
							}
						}
		...
		//std::cout << M.get_table(false).get_size();
		if (o_t - holder > 500)

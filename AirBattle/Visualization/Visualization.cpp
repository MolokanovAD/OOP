// Visualization.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int main() {
	...
	Button exit_button("Exit", l - 150, w - 50, font);
	...
	int X = 100, Y = 100;
	while (window.isOpen() && !exit_button.isPressed()) {
		...
		while (window.pollEvent(event)) {
			...
			if (event.type == Event::MouseButtonPressed)//сохранение
				if (event.key.code == Mouse::Left)
					if (exit_button.is_being_pressed(mpos.x, mpos.y))
						exit_button.press();
